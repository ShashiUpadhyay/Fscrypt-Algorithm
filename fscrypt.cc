...
BF_KEY* BF_SetKey_Encrypt(int indexIn, char* key_In, int enc_dec_scenario_In) {
	bf_key_global_ptr = (BF_KEY*) allocateMemory1D(1 * sizeof(BF_KEY));
	BF_set_key(bf_key_global_ptr, BLOCKSIZE, (const unsigned char*) (key_In));
	if (enc_dec_scenario_In == BF_ENCRYPT) {
		BF_ecb_encrypt((const unsigned char*) (textpostxor[indexIn]),
				cipherblocks[indexIn], bf_key_global_ptr, BF_ENCRYPT);
	} else if (enc_dec_scenario_In == BF_DECRYPT) {
		BF_ecb_encrypt((const unsigned char *) cipherblocks[indexIn],
				blocks[indexIn], bf_key_global_ptr, BF_DECRYPT);
	}
